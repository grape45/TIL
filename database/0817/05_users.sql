...
-- 데이터를 추가
.mode csv
-- 같은 디렉토리에 있는 user.csv 파일을 users 테이블로
...
-- 이씨 중에 나이가 가장 적은 사람의 이름과 계좌 잔고
SELECT MIN(age), first_name, balance FROM users WHERE last_name = '이';
-- 30세 이상인 사람들의 평균 나이
SELECT AVG(age) FROM users WHERE age >= 30;
-- 계좌 잔액(balance)이 가장 높은 사람과 그 액수
SELECT MAX(balance), first_name FROM uesrs;
-- 나이 30 이상인 사람의 계좌 평균 잔액
SELECT AVG(balance) FROM users WHERE age >= 30;

-- 지역번호가 02인 사람
SELECT COUNT(*) FROM users WHERE phone LIKE '02-%';
-- 준으로 끝나는 사람
SELECT COUNT(*) FORM users WHERE first_name LIKE '%준';
-- 중간번호가 5114
