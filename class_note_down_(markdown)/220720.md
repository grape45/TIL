...
- 인스턴스 메소드
  ...
  - 호출한 인스턴스르 의미하는 self 매개 변소를 통해 인스턴스를 조작
...
2. 상속
   ...
   - 다중 상속
     ...
     - 부모 클래스의 메소드 이름과 기본 기능은 그대로 사용가능하지만, 특정 기능을 바꾸고 싶을 때 사용
...
4. 캡슐화
   ...
   - **실제로 파이썬은 수단과 방법을 써서 접근 가능하기에 캡슐화를 거의 다루지 않음**
   - 파이썬에서 기능상으로 존재하진 않으나, 관용적으로 사용되는 표현은 존재
...
a = sorted(a)
# [3, 2, 1]
# 단지 함수임. input이 있으면 output을 도출할 뿐
